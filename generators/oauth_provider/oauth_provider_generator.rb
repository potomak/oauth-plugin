...
class OauthProviderGenerator < Rails::Generator::Base
  ...
  def manifest
    record do |m|
      ...
      m.route_name 'revoke', '/oauth/revoke', :controller=>'oauth', :action=>'revoke'
      ...
      if !options[:test_unit]
        ...
        m.template 'users.yml',    File.join('spec/fixtures',"users.yml")
        ...
        m.template 'users.yml',    File.join('test/fixtures',"users.yml")
